...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        # Slice the interval with bounds 
        temp = np.linspace(lower_bound, upper_bound, num_bins+1)
        # We only takes the range without the head and the tail
        return temp[1:-1]
        # End your code
    ...
    def discretize_observation(self, observation):
        ...
        """
        # Begin your code
        # Create list for storing the discretize value
        temp_list = []
        
        for i in range(len(observation)):
            # Discretize
            temp_list.append(self.discretize_value(observation[i], self.bins[i]))
        return temp_list
        # End your code

    def choose_action(self, state):
        """pre_qtable = np.load("./Tables/cartpole_table.npy")
            # if np.max(self.qtable[self.discretize_observation(self.env.reset())]) > np.max(pre_qtable[self.discretize_observation(self.env.reset())]):
    ...
    def learn(self, state, action, reward, next_state, times, done):
        """
        ...
        """
        # Begin your code
        # function for test the qtable everyr round when the traning progress is over 
        def test(input_table):
            testing_agent = Agent(env)
            # Change the filename to your student id
            testing_agent.qtable = input_table
            rewards = []
            for _ in range(100):
        ...
        # Update the qtable  
        if done:
            # If done, get rid of non-existent next state
            self.qtable[state[0], state[1], state[2],state[3], action] += self.learning_rate * (reward - self.qtable[state[0], state[1], state[2],state[3], action])
        ...
        #if traning progress in down, test the past qtable and the current qtable
        if times == 2999 and done:
            try:
                if test(np.load("./Tables/cartpole_table.npy")) < test(self.qtable):
                    # choose the one with the better reward
...
def train(env):
    ...
    for ep in tqdm(range(episode)):
        ...
        while True:
            ...
            training_agent.learn(state, action, reward, next_state, ep, done)
            # print(np.max(np.load("./Tables/cartpole_table.npy")[training_agent.discretize_observation(training_agent.env.reset())]))
            if done:
                rewards.append(count)
                break
